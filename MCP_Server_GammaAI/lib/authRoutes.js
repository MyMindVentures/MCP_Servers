/**
 * Auth API routes and JWT middleware. Use in both streamable-http and SSE apps.
 */
...
export function registerAuthRoutes(app) {
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    const user = authStore.findUserByUsername(login) || authStore.findUserByEmail(login);
  ...
  app.post("/api/auth/forgot-password", async (req, res) => {
    ...
    if (!user) {
      return res.json({ message: "If an account exists for this email, you will receive a reset link" });
    }
    const token = authStore.createResetToken(user.id);
    const baseUrl = process.env.MCP_PUBLIC_URL || `http://127.0.0.1:${process.env.PORT || 3001}`;
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    ...
    if (!authStore.updatePassword(userId, passwordHash)) return res.status(500).json({ error: "Failed to update password" });
    return res.json({ message: "Password has been reset" });
  });

  app.get("/api/auth/me", (req, res) => {
    if (!req.user) return res.status(401).json({ error: "Not authenticated" });
    return res.json({ user: { id: req.user.id, username: req.user.username, email: req.user.email } });
  });
  ...
  app.post("/api/auth/change-password", async (req, res) => {
    ...
    const passwordHash = auth.hashPassword(newPassword);
    if (!authStore.updatePassword(req.user.id, passwordHash)) return res.status(500).json({ error: "Failed to update password" });
