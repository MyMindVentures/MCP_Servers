/**
 * File-based user store and password reset tokens for auth.
 * Uses data/auth-data.json (or AUTH_DATA_PATH). Creates data dir if missing.
 */
...
const DATA_DIR = path.resolve(process.env.AUTH_DATA_PATH || path.join(__dirname, "..", "data"));
...
export function findUserByUsername(username) {
  ...
  return load().users.find((u) => u.usernameLower === normalized || u.username === username);
}

export function findUserByEmail(email) {
  const normalized = String(email).trim().toLowerCase();
  return load().users.find((u) => u.emailLower === normalized || u.email === email);
}
...
export function createUser({ username, email, passwordHash }) {
  ...
  if (data.users.some((u) => u.usernameLower === usernameLower)) return { ok: false, error: "Username already taken" };
  if (data.users.some((u) => u.emailLower === emailLower)) return { ok: false, error: "Email already registered" }
...
export function createResetToken(userId) {
  ...
  data.resetTokens[token] = { userId, expiresAt: Date.now() + RESET_TOKEN_EXPIRY_MS };
  save(data);
  return token;
}
